...
from ecs_mcp_server.errors import pending_actions
from ecs_mcp_server.tools.lifecycle import make_lifecycle_tools
...
# ============================================================
def test_power_action_start_returns_job_id(settings, mock_client):
    """start is non-destructive — executes immediately, no approval needed."""
    mock_client.batch_start_servers.return_value = MagicMock(job_id="job-1")
    ...
    mock_client.batch_start_servers.assert_called_once()
    mock_client.batch_stop_servers.assert_not_called()
    mock_client.batch_reboot_servers.assert_not_called()
...
def test_power_action_destructive_returns_pending_approval(settings, mock_client, action):
...
def test_power_action_stop_stores_correct_preview(settings, mock_client):
    tools = make_lifecycle_tools(settings)
...
def test_power_action_reboot_default_soft(settings, mock_client):
    tools = make_lifecycle_tools(settings)
    out = tools["ecs_power_action"](server_ids=[UUID_A], action="reboot")
    assert out["data"]["type"] == "SOFT"


def test_power_action_invalid_action_rejected(settings, mock_client):
    tools = make_lifecycle_tools(settings)
    out = tools["ecs_power_action"](server_ids=[UUID_A], action="halt")
    assert out["ok"] is False
    assert out["error"]["code"] == "INVALID_PARAMS"
    mock_client.batch_start_servers.assert_not_called()


def test_power_action_invalid_uuid_rejected(settings, mock_client):
    tools = make_lifecycle_tools(settings)
...
def test_power_action_start_propagates_huawei_client_exception(settings, mock_client):
    ...
    mock_client.batch_start_servers.side_effect = e
    tools = make_lifecycle_tools(settings)
    out = tools["ecs_power_action"](server_ids=[UUID_A], action="start")
    assert out["ok"] is False
...
# ============================================================
def test_confirm_destructive_executes_stop(settings, mock_client):
    """Confirming a stop approval actually executes the stop."""
    mock_client.batch_stop_servers.return_value = MagicMock(job_id="job-stop")
    tools = make_lifecycle_tools(settings)
    ...
    sent = mock_client.batch_stop_servers.call_args[0][0]
    assert sent.body.os_stop.type == "HARD"
    assert {s.id for s in sent.body.os_stop.servers} == {UUID_A, UUID_B}


def test_confirm_destructive_executes_reboot(settings, mock_client):
    """Confirming a reboot approval actually executes the reboot."""
    mock_client.batch_reboot_servers.return_value = MagicMock(job_id="job-rb")
    tools = make_lifecycle_tools(settings)
    ...
    sent = mock_client.batch_reboot_servers.call_args[0][0]
    assert sent.body.reboot.type == "SOFT"


def test_confirm_destructive_invalid_approval_id(settings, mock_client):
...
def test_confirm_destructive_double_use_rejected(settings, mock_client):
...
# ============================================================
def test_delete_returns_pending_approval(settings, mock_client):
    tools = make_lifecycle_tools(settings)
    out = tools["ecs_delete_server"](server_ids=[UUID_A])
    assert out["ok"] is True
    assert out["data"]["status"] == "pending_approval"
    assert out["data"]["action"] == "delete"
    mock_client.delete_servers.assert_not_called()


def test_delete_confirm_executes(settings, mock_client):
    mock_client.delete_servers.return_value = MagicMock(job_id="job-del")
    tools = make_lifecycle_tools(settings)
    ...
    sent = mock_client.delete_servers.call_args[0][0]
...
# ============================================================
def test_resize_returns_pending_approval(settings, mock_client):
    tools = make_lifecycle_tools(settings)
    ...
    mock_client.resize_server.assert_not_called()


def test_resize_confirm_executes(settings, mock_client):
    mock_client.resize_server.return_value = MagicMock(job_id="job-rsz")
    tools = make_lifecycle_tools(settings)
    ...
    sent = mock_client.resize_server.call_args[0][0]
