...
2. In dev mode (``MCP_GATEWAY_AUTH_MODE=dev``): synthesise an admin
   Identity for loopback callers and skip JWT verification + RBAC.
   Non-loopback callers are still rejected. A WARN log is emitted on
...
- Synthesises an Identity with ``sub="dev-local"`` and
  ...
  ``MCP_DEV_ROLES``).
- Only does this for loopback callers (127.0.0.1 / ::1). Anything else
  is still 403 — dev mode is not an open relay.
- Emits a ``WARNING`` log on every request.

This lets a developer run ``./start.sh`` locally without generating
JWTs, while keeping the safety net that the mode is never silently
active on a public-facing port.
"""
...
# Loopback hosts that are allowed in dev mode.
...
class GatewayAuthMiddleware:
    ...
    Supports three modes controlled by ``MCP_GATEWAY_AUTH_MODE``:

    - ``jwt`` (default when the env var is unset): full JWT verification.
    - ``dev``: skip JWT, synthesise admin identity for loopback only.
    - ``disabled``: **dangerous** — skip all auth. Only for isolated
      test environments. Emits a CRITICAL log on startup and every
      request.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if self._mode not in ("jwt", "dev", "disabled"):
            raise RuntimeError(
                f"MCP_GATEWAY_AUTH_MODE={self._mode!r} is invalid. "
                f"Use 'jwt', 'dev', or 'disabled'."
            )

        if self._mode == "disabled":
            log.critical(
                "⛔ MCP_GATEWAY_AUTH_MODE=disabled: ALL AUTHENTICATION IS OFF. "
                "Every request is allowed without any identity check. "
                "NEVER use this on a network-accessible port."
            )

        if self._mode == "dev":
            log.warning(
                "⚠ MCP_GATEWAY_AUTH_MODE=dev: JWT verification is DISABLED. "
                "Loopback callers receive a synthesised admin identity. "
                "Non-loopback callers are still rejected. "
    ...
    def _resolve_identity(self, scope: Scope, path: str) -> Identity:
        ...
        if self._mode == "disabled":
            log.warning("AUTH DISABLED: allowing request without identity")
            return Identity(sub="anonymous", roles=["admin"])
    ...
    def _resolve_dev(self, scope: Scope) -> Identity:
        """Dev mode: allow loopback callers with a synthesised identity.
        ...
        log.warning(
            "DEV MODE: synthesising identity sub=%s roles=%s path=%s — DO NOT USE IN PRODUCTION",
            self._dev_subject,
            self._dev_roles,
            scope.get("path", ""),
        )
    ...
    async def _reject(self, scope: Scope, send: Send, exc: AuthError) -> None:
        ...
        log.warning(
            "auth-rejected status=%d path=%s peer=%s reason=%s",
            exc.status,
            scope.get("path", ""),
            host,
            exc.reason,
