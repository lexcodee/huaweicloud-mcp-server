...
from pipeline_mcp_server.tools.execution import make_execution_tools
from pipeline_mcp_server.tools.lifecycle import make_lifecycle_tools

def test_pipeline_run_with_default_config(settings, mock_client):
    mock_client.run_pipeline.return_value = SimpleNamespace(
        pipeline_run_id="r-1",
    ...
    call = mock_client.run_pipeline.call_args
...
def test_pipeline_run_with_branch_override(settings, mock_client):
    mock_client.run_pipeline.return_value = SimpleNamespace(
        pipeline_run_id="r-2",
    ...
    assert result["ok"] is True
    body = mock_client.run_pipeline.call_args.args[0].body
...
def test_pipeline_set_status_enabled_calls_unban_endpoint(settings, mock_client):
    mock_client.do_http_request.return_value = True
    ...
    call = mock_client.do_http_request.call_args
...
def test_pipeline_set_status_enabled_executes_immediately(settings, mock_client):
    """status='enabled' is non-destructive — executes immediately."""
    mock_client.do_http_request.return_value = True
    tool = make_lifecycle_tools(settings)["pipeline_set_status"]
    result = tool(pipeline_id="p1", status="enabled")
    assert result["ok"] is True
    mock_client.do_http_request.assert_called_once()


def test_pipeline_set_status_disabled_returns_pending_approval(settings, mock_client):
    ...
    mock_client.do_http_request.assert_not_called()


def test_pipeline_set_status_disabled_confirm_executes(settings, mock_client):
...
def test_pipeline_confirm_destructive_invalid_id(settings, mock_client):
    tools = make_lifecycle_tools(settings)
...
def test_pipeline_set_status_invalid_status_rejected(settings, mock_client):
    tool = make_lifecycle_tools(settings)["pipeline_set_status"]
