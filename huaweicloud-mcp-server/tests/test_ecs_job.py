...
from huaweicloud_mcp.services.ecs.tools.job import make_job_tools


def test_job_status_success(settings, mock_ecs_client):
    sub = MagicMock(
    ...
    mock_ecs_client.show_job.return_value = resp
...
def test_job_status_fail_carries_error(settings, mock_ecs_client):
    resp = MagicMock(
...
def test_job_status_missing_job_id(settings, mock_ecs_client):
    tools = make_job_tools(settings)
