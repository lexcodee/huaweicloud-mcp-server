...
from huaweicloud_mcp.services.pipeline.tools.query import make_query_tools


def test_pipeline_list_returns_compact_envelope(settings, mock_pipeline_client):
    ...
    mock_pipeline_client.list_pipelines.return_value = fake_resp
...
def test_pipeline_list_rejects_unknown_status(settings, mock_pipeline_client):
    tool = make_query_tools(settings)["pipeline_list"]
    result = tool(status=["BANANA"])
    assert result["ok"] is False
    assert result["error"]["code"] == "INVALID_PARAMS"
    mock_pipeline_client.list_pipelines.assert_not_called()


def test_pipeline_get_detail_decodes_definition_string(settings, mock_pipeline_client):
    ...
    mock_pipeline_client.show_pipeline_detail.return_value = fake_detail
...
def test_missing_project_id_when_no_default(monkeypatch, mock_pipeline_client):
