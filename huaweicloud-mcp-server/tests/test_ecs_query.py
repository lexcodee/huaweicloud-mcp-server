...
from huaweicloud_mcp.services.ecs.tools.query import make_query_tools
...
def test_list_servers_passes_filters_and_returns_compact(settings, mock_ecs_client):
    resp = MagicMock()
    resp.servers = [_fake_server()]
    resp.count = 1
    mock_ecs_client.list_servers_details.return_value = resp
    ...
    sent_req = mock_ecs_client.list_servers_details.call_args[0][0]
...
def test_list_servers_drops_null_fields_for_token_efficiency(settings, mock_ecs_client):
...
def test_list_servers_invalid_status_rejected(settings, mock_ecs_client):
    tools = make_query_tools(settings)
...
def test_get_server_full_invalid_uuid(settings, mock_ecs_client):
    tools = make_query_tools(settings)
...
def test_get_server_full_not_found(settings, mock_ecs_client):
...
def test_get_server_invalid_detail_level(settings, mock_ecs_client):
    tools = make_query_tools(settings)
    out = tools["ecs_get_server"](server_id=VALID_UUID, detail_level="verbose")
    assert out["ok"] is False
    assert out["error"]["code"] == "INVALID_PARAMS"
    mock_ecs_client.list_servers_details.assert_not_called()
    mock_ecs_client.show_server.assert_not_called()


def test_get_server_full_returns_rich_fields(settings, mock_ecs_client):
...
def test_get_server_status_compact(settings, mock_ecs_client):
    show_resp = MagicMock()
    show_resp.server = _fake_server()
    mock_ecs_client.show_server.return_value = show_resp
    ...
    mock_ecs_client.show_server.assert_called_once()
    mock_ecs_client.list_servers_details.assert_not_called()


def test_list_flavors_applies_client_side_limit(settings, mock_ecs_client):
    resp = MagicMock()
    fs = []
    for i in range(5):
        ...
        fs.append(f)
    resp.flavors = fs
    mock_ecs_client.list_flavors.return_value = resp
