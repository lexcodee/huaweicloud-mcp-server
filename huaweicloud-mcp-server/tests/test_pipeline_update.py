...
from huaweicloud_mcp.services.pipeline.tools.update import make_update_tools
from huaweicloud_mcp.services.pipeline.tools.lifecycle import make_lifecycle_tools


def _captured_dto(mock_pipeline_client):
    """Pull the PipelineDTO body out of the captured update_pipeline_info call."""
    call = mock_pipeline_client.update_pipeline_info.call_args
...
def test_update_default_branch_only_preserves_other_fields(
    settings, mock_pipeline_client, sample_pipeline_detail
):
    mock_pipeline_client.show_pipeline_detail.return_value = sample_pipeline_detail
    mock_pipeline_client.update_pipeline_info.return_value.to_dict = lambda: {
    ...
    dto = _captured_dto(mock_pipeline_client)
...
def test_update_returns_pending_approval_not_execute(settings, mock_pipeline_client, sample_pipeline_detail):
    ...
    mock_pipeline_client.update_pipeline_info.assert_not_called()


def test_refuses_when_no_field_to_update(settings, mock_pipeline_client):
    tool = make_update_tools(settings)["pipeline_update_info"]
    result = tool(pipeline_id="pipeline-uuid")
    assert result["ok"] is False
    assert result["error"]["code"] == "NO_FIELDS_TO_UPDATE"
    mock_pipeline_client.show_pipeline_detail.assert_not_called()
    mock_pipeline_client.update_pipeline_info.assert_not_called()


def test_refuses_invalid_pre_task_value(settings, mock_pipeline_client):
    tool = make_update_tools(settings)["pipeline_update_info"]
    result = tool(
        pipeline_id="pipeline-uuid",
...
def test_refuses_when_alias_required_for_multi_source(settings, mock_pipeline_client):
    ...
    mock_pipeline_client.show_pipeline_detail.return_value = detail
    tool = make_update_tools(settings)["pipeline_update_info"]
