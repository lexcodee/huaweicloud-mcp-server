...
export HUAWEICLOUD_REGION=af-south-1
...
| `HUAWEICLOUD_PROJECT_ID` | ECS/CTS | `15f2d47addb14784b82eb910447250a9` | Project UUID |
...
`cts-mcp-server`) are preserved unchanged. The unified package supersedes them:
...
├── ecs-mcp-server/              # (legacy) standalone ECS package
├── codearts-pipeline-mcp-server/# (legacy) standalone Pipeline package
└── cts-mcp-server/              # (legacy) standalone CTS package
```
